...
/**
 * Parse video-embed includes and structured content from the SurrealDB page markdown.
 */
function parseVideosFromContent(content: string): {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Detect video embeds: {% include video-embed.html provider="youtube" id="..." title="..." caption="..." %}
    ...
    // Parse metadata lines after a video embed
    if (currentVideo) {
      ...
      // End of video block on separator
      if (line.startsWith("---")) {
...
/**
 * Also find pages that reference YouTube/video content across the whole knowledge base.
 */
...
export default async function VideosPage() {
  ...
  return (
    <div className="wide-wrapper">
          ...
          >
            {vids.map((video) => (
              <div key={video.id} className="video-card">
                {/* YouTube thumbnail — links to YouTube */}
                <a
      ...
      {/* Text content from the learning videos page (channel references etc.) */}
      {learningContent && (
      ...
      {/* Cross-references to other pages with video content */}
      {crossRefs.length > 0 && (
...
/**
 * Clean video-embed includes from markdown content.
 */
