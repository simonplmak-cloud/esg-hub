...
| F12 MCP config | opencode.json: github→`SIMONPLMAK_CLOUD_PAT`, brave-search/google-search/browserless enabled (+120s startup timeout for npx cold starts on this slow machine), vercel remote added; browserless container started (Chrome 145) | **github ✓ verified 2026-07-19** (get_me → simonplmak-cloud; get_file_contents on private repo → content). **browserless ✓ verified** (MCP screenshot of prod). brave/google-search: servers confirmed startable via npx (cold start ~60s); live check pending next restart. vercel: needs OAuth consent | 2026-07-19 |
...
2. **Restart opencode once more** (config gained 120s MCP startup timeouts) — then verify brave-search/google-search live queries and run `opencode mcp auth vercel` for the OAuth consent (AC-14/AC-15 final checks). github + browserless already verified ✓
