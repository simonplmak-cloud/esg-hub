...
| F12 MCP config | opencode.json: github→`SIMONPLMAK_CLOUD_PAT`, brave-search/google-search/browserless enabled, vercel remote added; browserless container started (Chrome 145) | **browserless ✓ verified live via MCP** (screenshot of prod site, 2026-07-19). github/brave/google/vercel MCPs: config correct but **pending opencode relaunch from a fresh shell** — current session's env predates the bashrc exports, so `{env:SIMONPLMAK_CLOUD_PAT}` resolves empty and github MCP falls back to the old humanity4ai token (proven: get_file_contents → 404, search_code → 0, get_me → humanity4ai) | 2026-07-19 |
...
2. **Relaunch opencode from a fresh terminal** (quit fully, new shell, restart) so the new bashrc exports reach the MCP servers — then verify: github MCP repo read (AC-A2), brave-search/google-search live query (AC-14), vercel MCP OAuth consent (AC-15). browserless already verified ✓
