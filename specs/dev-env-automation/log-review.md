...
| F12 MCP config | opencode.json: github→`SIMONPLMAK_CLOUD_PAT`; brave-search/google-search **globally installed** (`~/.npm-global`) with direct `node <bin>` commands — npx cold-start (~60s on this slow machine) was causing opencode's MCP handshake to fail; browserless enabled; vercel remote added; browserless container (Chrome 145) | **github ✓** (get_me → simonplmak-cloud; repo file read ✓). **browserless ✓** (MCP screenshot). brave/google: binaries smoke-tested (instant `initialize` response); live check pending next restart. vercel: `needs_auth` — OAuth consent | 2026-07-19/20 |
...
2. **Restart opencode** so the search MCPs load from their global binaries (instant start) — verify brave-search/google-search live queries; then run `opencode mcp auth vercel` for the OAuth consent (AC-14/AC-15 final checks). github + browserless already verified ✓
